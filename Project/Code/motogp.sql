...
create table rennfahrer_saison (
    id serial,
    saison_id bigint unsigned not null,
    rennfahrer_id bigint unsigned not null,
...
insert into motorrad(marke) values
 ('Ducati')
,('Suzuki')
,('Petronas SRT')
,('Pramac')
...
insert into rennfahrer_saison(rennfahrer_id,saison_id) values
 (5,1)
,(2,1)
,(4,1)
,(1,1)
...
select 
	...
	AVG(rangliste.punkte) AS AVG_Saison_1
from 
    rennfahrer
...
select 
    sponsor.name as Sponsor,
	AVG(rangliste.punkte) AS AVG_Saison_2
from 
    rennfahrer
	 inner join motorrad on rennfahrer.motorrad_id = motorrad.id
        inner join motorrad_sponsor on motorrad.id = motorrad_sponsor.id
